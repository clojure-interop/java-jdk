...
(defn set-round-rect
  ...
  ([^RoundRectangle2D$Double this ^Double x ^Double y ^Double w ^Double h ^Double arcw ^Double arch]
    (-> this (.setRoundRect x y w h arcw arch)))
  ([^RoundRectangle2D$Double this ^java.awt.geom.RoundRectangle2D rr]
...
(defn get-bounds-2-d
  ...
  (^java.awt.geom.Rectangle2D [^RoundRectangle2D$Double this]
    (-> this (.getBounds2D))))
...
(defn empty?
  ...
  (^Boolean [^RoundRectangle2D$Double this]
    (-> this (.isEmpty))))
