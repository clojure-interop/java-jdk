...
(defn ->robot
  ...
  (^Robot [^java.awt.GraphicsDevice screen]
    (new Robot screen))
  (^Robot []
    (new Robot )))
...
(defn get-pixel-color
  ...
  (^java.awt.Color [^Robot this ^Integer x ^Integer y]
...
(defn create-screen-capture
  ...
  (^java.awt.image.BufferedImage [^Robot this ^java.awt.Rectangle screen-rect]
...
(defn mouse-move
  ...
  x - X position - `int`
  y - Y position - `int`"
  ([^Robot this ^Integer x ^Integer y]
    (-> this (.mouseMove x y))))
