...
(defn point-to-path
  ...
  point - the point to convert. If it is not the same object as location, point will remain unmodified by this call. - `java.awt.geom.Point2D`
  location - a Point2D to hold the returned location. It can be the same object as point. - `java.awt.geom.Point2D`
...
(defn path-to-point
  ...
  location - a Point2D representing the advance (in x) and offset (in y) of a location relative to the path. If location is not the same object as point, location will remain unmodified by this call. - `java.awt.geom.Point2D`
  preceding - if true, the portion preceding the advance should be used, if false the portion after should be used. This has no effect if the path does not break or bend sharply at the advance. - `boolean`
