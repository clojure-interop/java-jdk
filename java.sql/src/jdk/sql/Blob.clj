...
(defn get-bytes
  ...
  ([^Blob this ^Long pos ^Integer length]
...
(defn get-binary-stream
  ...
  (^java.io.InputStream [^Blob this ^Long pos ^Long length]
...
(defn position
  ...
  (^Long [^Blob this pattern ^Long start]
...
(defn set-bytes
  ...
  (^Integer [^Blob this ^Long pos bytes ^Integer offset ^Integer len]
    (-> this (.setBytes pos bytes offset len)))
  (^Integer [^Blob this ^Long pos bytes]
...
(defn set-binary-stream
  ...
  throws: java.sql.SQLException - if there is an error accessing the BLOB value or if pos is less than 1"
  (^java.io.OutputStream [^Blob this ^Long pos]
