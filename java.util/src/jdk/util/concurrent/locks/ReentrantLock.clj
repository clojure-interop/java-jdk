...
(defn try-lock
  ...
  (^Boolean [^ReentrantLock this ^Long timeout ^java.util.concurrent.TimeUnit unit]
...
(defn has-waiters?
  ...
  (^Boolean [^ReentrantLock this ^java.util.concurrent.locks.Condition condition]
...
(defn new-condition
  ...
  (^java.util.concurrent.locks.Condition [^ReentrantLock this]
    (-> this (.newCondition))))
...
(defn to-string
  ...
  (^java.lang.String [^ReentrantLock this]
    (-> this (.toString))))
...
(defn has-queued-thread?
  ...
  (^Boolean [^ReentrantLock this ^java.lang.Thread thread]
...
(defn get-wait-queue-length
  ...
  throws: java.lang.IllegalMonitorStateException - if this lock is not held"
  (^Integer [^ReentrantLock this ^java.util.concurrent.locks.Condition condition]
