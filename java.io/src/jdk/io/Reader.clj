...
(defn read
  ...
  (^Integer [^Reader this cbuf ^Integer off ^Integer len]
    (-> this (.read cbuf off len)))
  (^Integer [^Reader this ^java.nio.CharBuffer target]
    (-> this (.read target)))
  (^Integer [^Reader this]
    (-> this (.read))))
...
(defn mark
  ...
  ([^Reader this ^Integer read-ahead-limit]
