...
(defn ->char-array-reader
  ...
  (^CharArrayReader [buf ^Integer offset ^Integer length]
...
(defn read
  ...
  (^Integer [^CharArrayReader this b ^Integer off ^Integer len]
    (-> this (.read b off len)))
  (^Integer [^CharArrayReader this]
    (-> this (.read))))
...
(defn skip
  ...
  (^Long [^CharArrayReader this ^Long n]
    (-> this (.skip n))))
...
(defn mark
  ...
  throws: java.io.IOException - If an I/O error occurs"
  ([^CharArrayReader this ^Integer read-ahead-limit]
