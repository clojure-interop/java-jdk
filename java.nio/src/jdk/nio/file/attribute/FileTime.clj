...
(defn *from
  ...
  (^java.nio.file.attribute.FileTime [^Long value ^java.util.concurrent.TimeUnit unit]
    (FileTime/from value unit))
  (^java.nio.file.attribute.FileTime [^java.time.Instant instant]
...
(defn *from-millis
  ...
  returns: a FileTime representing the given value - `java.nio.file.attribute.FileTime`"
  (^java.nio.file.attribute.FileTime [^Long value]
