...
(defn provider
  ...
  (^java.nio.channels.spi.SelectorProvider [^Selector this]
    (-> this (.provider))))
...
(defn select
  ...
  throws: java.io.IOException - If an I/O error occurs"
  (^Integer [^Selector this ^Long timeout]
    (-> this (.select timeout)))
  (^Integer [^Selector this]
    (-> this (.select))))
...
(defn wakeup
  ...
  (^java.nio.channels.Selector [^Selector this]
    (-> this (.wakeup))))
