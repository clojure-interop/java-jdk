...
(defn ->rmi-connector
  ...
  environment - additional attributes specifying how to make the connection. For JNDI-based addresses, these attributes can usefully include JNDI attributes recognized by InitialContext. This parameter can be null, which is equivalent to an empty Map. - `java.util.Map`

  throws: java.lang.IllegalArgumentException - if url is null."
  (^RMIConnector [^javax.management.remote.JMXServiceURL url ^java.util.Map environment]
...
(defn to-string
  ...
  (^java.lang.String [^RMIConnector this]
    (-> this (.toString))))
...
(defn get-address
  ...
  (^javax.management.remote.JMXServiceURL [^RMIConnector this]
    (-> this (.getAddress))))

(defn connect
  "Description copied from interface: JMXConnector

  environment - the properties of the connection. Properties in this map override properties in the map specified when the JMXConnector was created, if any. This parameter can be null, which is equivalent to an empty map. - `java.util.Map`
  ...
  ([^RMIConnector this ^java.util.Map environment]
...
(defn get-m-bean-server-connection
  ...
  (^javax.management.MBeanServerConnection [^RMIConnector this ^javax.security.auth.Subject delegation-subject]
    (-> this (.getMBeanServerConnection delegation-subject)))
  (^javax.management.MBeanServerConnection [^RMIConnector this]
...
(defn add-connection-notification-listener
  ...
  ([^RMIConnector this ^javax.management.NotificationListener listener ^javax.management.NotificationFilter filter ^java.lang.Object handback]
...
(defn remove-connection-notification-listener
  ...
  ([^RMIConnector this ^javax.management.NotificationListener listener]
