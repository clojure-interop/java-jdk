...
(defn open
  ...
  ([^TargetDataLine this ^javax.sound.sampled.AudioFormat format ^Integer buffer-size]
    (-> this (.open format buffer-size)))
  ([^TargetDataLine this ^javax.sound.sampled.AudioFormat format]
    (-> this (.open format))))
...
(defn read
  ...
  (^Integer [^TargetDataLine this b ^Integer off ^Integer len]
    (-> this (.read b off len))))
